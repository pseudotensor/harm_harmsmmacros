plc    17	# plc <file> <function> <type of plot=100,000,overlay=010,000,limits=000,001> <0 0 0 0>
		...
		if(($SOLIDCONTOURS==0)&&($min*$max < 0.)) {
                        echo "pt:1"
		        if($cres>1) {define delta (($max-$min)/($cres-1)) }
		        if($cres==1) {define delta ($cres) }
			 ...
			 levels lev
		         ...
		         #
		         ctype default
		         if($cres % 2 == 1){ set lev=$min,0,$delta }
		         if($cres % 2 == 0){ set lev=$min,0,$delta }
		         levels lev
		         ltype $NEGCONTLTYPE
		         ctype $NEGCONTCOLOR
		         set image[fix,fiy] = newfun
		...
		if(($min<=0)&&($max<=0)) {
                        echo "pt:2"
		        if($cres>0) { set lev=$min,$max,($max-$min)/($cres-1) define delta (($max-$min)/($cres-1)) }
		        if($cres==1) { set lev=$cres }
		...
		if(!(thebits & 0x010)) {\
		 ...
		 if($?LOGTYPE){\
                  #
		  if($LOGTYPE==0 ) {\
		   echo "doing logtype==0"
		   ticksize 0 0 0 0
                   box
		  }
                  #
		  if($LOGTYPE==1 ) {\
		   echo "doing logtype==1"
		   ticksize -1 0 0 0
                   box
		  }
                  #
		  if($LOGTYPE==2) {\
		   echo "doing logtype==2"
                   #define truelimtxl (LG($txl))
                   #define truelimtxh (LG($txh))
                   #define truelimtyl (LG($tyl))
                   #define truelimtyh (LG($tyh))
                   #limits $truelimtxl  $truelimtxh  $truelimtyl  $truelimtyh
		   ticksize -1 0 -1 0
		   box
		  }
                 }
		...
		#
		 ...
		 if($PLANE==2) {
		   ...
		   define yh ($Sz+$Lz)
                   ...
                   define tyl ($yl)
                   define tyh ($yh)
                   set newfun=inputfun if(j==$WHICHLEV)
                   define rxl (0)
                   define rxh ($nx)
                   define ryl (0)
                   define ryh ($nz)
...
plc0 17      # as with plc
		...
		contour
		#
plclg   17      # as with plc
		#
		set fun=$2
		set mybad=sqrt(-1)+funplot*0
		set mylgfunplus=(fun>0.0) ? lg(fun) : mybad
		set mylgfunminus=(fun<0.0) ? lg(-fun) : mybad
		#
		#
		if($?3 == 1) { define tobebits ($3) } else { define tobebits (0x0) }
                if('$tobebits'=='000') {
		...
		#
		   ...
		   plc $1 mylgfunplus $3 $4 $5 $6 $7
		   define POSCONTCOLOR blue
		   define NEGCONTCOLOR cyan
		   plc $1 mylgfunminus 011 $4 $5 $6 $7
		   #lweight 0
		   #plc $1 (LG(abs(fun))) 011 $4 $5 $6 $7
		   #lweight 3
		}
                if(!(thebits & 0x001)) {
                 if(thebits & 0x100) {\
		   ...
		   define POSCONTCOLOR blue
		   define NEGCONTCOLOR cyan
		   plc $1 mylgfunminus 110
		   #lweight 0
		   #plc $1 (LG(abs(fun))) 110
		   #lweight 3
		 }
                 if(thebits & 0x010) {\
		   define BOXCOLOR default
		   ...
		   define POSCONTCOLOR blue
		   define NEGCONTCOLOR cyan
		   plc $1 mylgfunminus 010
		   #lweight 0
		   #plc $1 (LG(abs(fun))) 010
		   3lweight 3
		 }
		 if(!((thebits & 0x010)||(thebits & 0x100))) {\
		   define BOXCOLOR default
...
animvpl	19	# animvpl 'dump' v 1 12 10  <0 0 0 0>
		...
		#
animvplplot 19	# like animvpl but outputs .eps's
                #defaults
                if($?5 == 0) { define numsend (4) }\
                else{\
                  if($?6 == 1) { define numsend (6) } else { define numsend 5 }
		...
		#
                ...
                do ii=0,$NUMDUMPS-1,$ANIMSKIP {
                  ...
                  define filename (_fname)
		  #
		  device postencap movie/$filename.eps
		  #
                  if($numsend==6){\
 		   vpl $filename $2 $3 $4 $5 $6 $7 $8 $9
                  }\
...
maskit    2    #
		...
		do ii=0,$nx*$ny*$nz-1,1{
		   ...
		   # want the x,y,z condition to not fudgeing matter if nx,ny,nz=1
