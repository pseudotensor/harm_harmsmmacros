...
#include "paraenohybrid.h"


int paraenohybrid_line_c2e( int whichquantity, int dir, int do_weight_or_recon, int whichreduce, int preforder, int pl, int bs, int ps, int pf, int bf, int *minorderit, int *maxorderit, int *shiftit, 
		  ...
		  FTYPE *Pindicator, FTYPE *yin, FTYPE *yout_left, FTYPE *yout_right ) 
{
  // JCM STUFF START
  int paraprocess_line_c2e( int whichquantity, int dir, int do_weight_or_recon, int whichreduce, int preforder, int pl, int bs, int ps, int pf, int bf, int *minorderit, int *maxorderit, int *shiftit, 
...
#define MIXEND 0.4
    ...
    stiffnonlocal=0.0;
    for(mm=-1;mm<=1;mm++){
      if(i+mm>=bs+1 && i+mm<=bf-1) stiffnonlocal=max(stiffnonlocal,stiffindicator[i+mm]);
    }
  ...
  if(FULLHYBRID==1 && !allpara || FULLHYBRID==0 || PARAMODWENO==0){
    returnedvalue=eno_line_reconstruct( whichquantity, do_weight_or_recon, CVT_C2E, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit, shockindicator,  df, dP, monoindicator, Pindicator, yin, yout_left, yout_right ); 
  }
  ...
  if(PARAMODWENO && MAXBND>=4){ // then assume can store enough dq's for PARA-like steepen/flat/monocheck
    ...
    if(FULLHYBRID==1 && !allweno || FULLHYBRID==0){
      paraprocess_line_c2e( whichquantity, dir, do_weight_or_recon, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit,  shockindicator, stiffindicator, parafrac, V, P, df, dP, etai, monoindicator, Pindicator, yin, yout_left, yout_right );  
    }
  }
...
#error If Steepen in paraline, must turn on CONTACTINDICATOR
    ...
    parasteepgen(pl,etai[i],&V[i],&P[i],&yin[i],&df[DFMONO][i],&face[0][i],&face[1][i]);
#endif
    ...
    //dqrange=preforder-2;
    dqrange=5;
    checkparamonotonicity(dqrange, pl, &yin[i], &df[DF2OFMONO][i], &df[DFMONO][i],&face[0][i],&face[1][i],&face[0][i],&face[1][i]);
#endif
