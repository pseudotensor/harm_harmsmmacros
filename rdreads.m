		...
		if(!($failuremode)){\
		  ...
		  define oldfiletype2 $filetype
	 	  rd np$1 -1
                ...
                #
rdbondi   0     # (must load rdbasic still first for gam)(why the fudge is this if statement not same as others in back slashes!)
                da 0_analdata2.dat
...
rdcalcs1 1      #
		...
		#
		       ...
		       if($?averywind==1){\
		            if($averywind){\
		               # for AVERYWIND PROBLEM
		               set GM = 0.201384588
		               set radius=sqrt(x12**2+x22**2+x32**2)
		               set pot$!!ext = -GM/radius
		            }
		           }
		       #
		       #
