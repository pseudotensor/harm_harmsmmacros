...
rmsvstime   0   #
		...
		do ii=0,10,1 {
		   ...
		   rdturb $filename
		   turbrms
		   set rmskevst[$ii]=rmske
		   set rmsbevst[$ii]=rmsbe
...
plotrms     0   #
		...
		ltype 0 ctype cyan pl 0 trms rmsrhovst 0010
		ltype 1 ctype red pl 0 trms rmsvvst 0010
		ltype 0 ctype green pl 0 trms rmsvxvst 0010
		ltype 1 ctype default pl 0 trms rmsvyvst 0010
		ltype 2 ctype default pl 0 trms rmsvzvst 0010
		ltype 1 ctype red pl 0 trms rmsbvst 0010
		ltype 0 ctype blue pl 0 trms rmsbxvst 0010
		ltype 1 ctype blue pl 0 trms rmsbyvst 0010
		ltype 2 ctype blue pl 0 trms rmsbzvst 0010
		ltype 0 ctype yellow pl 0 trms rmskevst 0010
		ltype 1 ctype yellow pl 0 trms rmsbevst 0010
turbrms     0   #
		set size=$nx*$ny*$nz
		set vsq=vx**2+vy**2+vz**2
		set bsq=bx**2+by**2+bz**2
		set ke=0.5*rho*vsq
		set be=0.5*bsq
		set kea=SUM(ke)/size
		set bea=SUM(be)/size
                ...
                set bya=SUM(by)/size
                set bza=SUM(bz)/size
		print {kea bea rhoa vxa vya vza bxa bya bza}
		#
		set rmske=SUM((ke-kea)*(ke-kea))/size
		set rmsbe=SUM((be-bea)*(be-bea))/size
